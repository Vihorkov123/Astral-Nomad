...
def slide(footer=True):
    ...
    N[0] += 1
    if footer:
        txt(s, 0.55, 7.02, 8, 0.4, [[(f'{N[0]:02d} — ASTRAL NOMAD', 10, MUTED, False)]])
...
txt(s, 0.9, 4.35, 11.5, 0.9, [
    [('Браузерный rogue-lite про выживание после крушения:', 17, TEXT, False)],
    [('исследуй сектора, собирай ресурсы, чини капсулу.', 17, TEXT, False)],
])
...
roles = [
    ...
    ('Гейм-дизайнер', '________________\nРИ-XXXXXX', 'механики · баланс'),
    ('Аналитик', '________________\nРИ-XXXXXX', 'ЦА · рынок · конкуренты'),
    ('Сценарист', '________________\nРИ-XXXXXX', 'сюжет · тексты'),
    ('Разработчик', '________________\nРИ-XXXXXX', 'код · сборка · тесты'),
]
...
num_card(s, 0.55, 1.85, 6.0, 4.7, '01', 'Мало хороших игр в жанре', [
    'В браузере почти нет нормальных rogue-lite: в основном кликеры и простые аркады.',
    'Игроки знают жанр по Vampire Survivors и Brotato, но те живут в Steam и требуют установки.',
    'Интересных проектов в нише на платформе мало.',
])
num_card(s, 6.8, 1.85, 6.0, 4.7, '02', 'Свободная ниша', [
    'Лёгкого браузерного rogue-lite с понятным циклом на Яндекс Играх почти нет.',
    'Аудитория платформы растёт, а жанр не занят.',
    'Это и есть наша возможность.',
], accent=CORAL)
...
num_card(s, 0.55, 1.85, 6.0, 4.7, 'B2C', 'Игрок коротких сессий', [
    '12–25 лет: школьники, студенты, молодые специалисты за ПК.',
    'Знаком с жанром по Vampire Survivors и Brotato, любит «ещё один заход».',
    'Хочет: понятный геймплей, прогресс без обнуления, сессию 5–15 минут.',
    'Не любит: терять весь прогресс при смерти и тратить время впустую.',
])
num_card(s, 6.8, 1.85, 6.0, 4.7, 'EDU', 'Начинающий разработчик', [
    'Студенты IT-направлений и инди-разработчики.',
    'Ищут рабочий пример полного цикла: от прототипа до публикации на платформе.',
    'Получают: открытый репозиторий с кодом, концептом и автотестами.',
], accent=CORAL)
...
txt(s, 0.85, 2.55, 5.0, 3.9, [
    [('Разработать и подготовить к публикации на Яндекс Играх rogue-lite Astral Nomad', 15, TEXT, True)],
    [('с сессией 5–15 минут и полным соответствием регламентам платформы — за 4 недели.', 13, MUTED, False)],
], spacing=8)
tasks = [
    'Анализ ЦА, платформы и конкурентов → УТП',
    'Концепт, сценарий, баланс 5 секторов',
    'MVP на Vanilla JS + Canvas (12 модулей)',
    'Интеграция SDK платформы',
...
# ===== 8. Ядро геймплея =====
s = slide(); header(s, 'как играется', 'Игровой цикл')
loop = [
...
txt(s, 0.55, 5.4, 12.2, 1.0, [
    [('Смерть — не конец: rogue-lite сохраняет сектора и оружие, забирая еду и часть лома.', 15, TEXT, False)],
], PP_ALIGN.CENTER, spacing=4)

# ===== 9. УТП =====
s = slide(); header(s, 'чем отличаемся', 'Уникальность проекта')
usp = [
    ('Риск как выбор', 'шанс охраны виден до вскрытия — решает игрок, а не случай'),
    ('Честный бой', 'перед атакой враг замахивается — удар видно и можно увернуться'),
    ('Смерть без злости', 'rogue-lite: прогресс секторов и оружие не сгорают'),
    ('Сюжет с выбором', 'память возвращается по секторам, 2 концовки + бесконечный режим'),
    ('0,1 МБ', 'графика — код на Canvas, звук — синтез: ноль ассетов'),
    ('Готов к платформе', 'русский язык, 12+, адаптив 21:9 / 16:9 / 4:3'),
]
...
s = slide(); header(s, 'что получили', 'Результат')
    ...
    'MVP Astral Nomad: 5 секторов, босс, 2 концовки, бесконечный режим',
    'Интеграция Yandex SDK и сохранение прогресса',
    'Сборка ≈0,1 МБ, автономная, проверена в Chrome / Firefox / Safari / Edge',
    '3 итерации плейтестов: правки баланса по отзывам игроков',
...
txt(s, 0.9, 4.6, 11.5, 2.0, [
    ...
    [('Гейм-дизайнер — ____________________ РИ-XXXXXX', 13, TEXT, False)],
    [('Аналитик — ____________________ РИ-XXXXXX', 13, TEXT, False)],
    [('Сценарист — ____________________ РИ-XXXXXX', 13, TEXT, False)],
    [('Разработчик — ____________________ РИ-XXXXXX', 13, TEXT, False)],
], spacing=2)
txt(s, 0.9, 6.55, 11.5, 0.5, [[('Astral Nomad · Яндекс Игры · 2026', 12, TEAL, False)]])

prs.save('/tmp/Astral_Nomad_Презентация.pptx')
